...
export const getPromptById = async (token: string, id: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/prompts/${id}`, {
		method: 'GET',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const updatePromptById = async (token: string, prompt: PromptItem) => {
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/prompts/${prompt.id}/update`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
